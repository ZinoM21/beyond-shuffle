...
def filter_by_heuristics(
    df: pd.DataFrame, heuristics: List[Tuple[Callable, Dict]]
) -> pd.DataFrame:
    mask = df.apply(
        lambda row: all(func(row, **kwargs) for func, kwargs in heuristics), axis=1
    )
    return df[mask].copy()
...
def generate_playlists(
    ...
    """
    Generate playlists using preset features and scoring. Deduplicate tracks and enforce artist diversity.
    """
    ...
    for name in playlists_to_generate:
        ...
        if name_title_case in presets:
            ...
            heuristics = preset["heuristics"]
            playlist_df = filter_by_heuristics(df, heuristics)
            ...
            tracks_with_artists = list(zip(playlist_df["track"], playlist_df["artist"]))
            deduplicated_tracks = get_unique_tracks(tracks_with_artists)
            diverse_tracks = enforce_artist_diversity(
                deduplicated_tracks, max_per_artist=max_per_artist
            )

            top_n_tracks = diverse_tracks[:num_songs]

            generated_playlists[name_title_case] = top_n_tracks
